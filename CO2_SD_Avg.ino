...
void loop () {
  ...
  while (millis() - t0 < 10000) {
    ...
    CO2_ppm += int((CO2_V - 0.4)*3125);   // 10-bits ADCs like the Arduino Uno (remember then to also
    Count += 1;                           // comment the analogReadResolution command in the setup).
  ...
  if (CO2_V==0) {
    ...
    dataString += ',';
    dataString += String(now.unixtime());  // Records the current time in seconds, calculated from
    dataString += ',';                     // a universal Unix-based time reference. Allows for
    dataString += String(CO2_ppm);         // simple time conversions if needed.
